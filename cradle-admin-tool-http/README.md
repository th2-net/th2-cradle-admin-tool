...
```
Help: this script provide ability to update comment for pages covered by time rage
 Arg: --cradle-admin-tool-url (required) - cradle admin tool URL
 Arg: --book (required) - th2 book for searching and updating pages
 Arg: --start-timestamp (required) - start timestamp for searching page to add --comment comment
 Arg: --end-timestamp (required) - end timestamp for searching page to add --comment comment
 Arg: --comment (required) - comment for adding to pages found from --start-timestamp to --end-timestamp
 Arg: --mode (optional) - work mode. Default value is 'append'
